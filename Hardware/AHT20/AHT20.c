...
u8 AHT20_Init()
{
    ...
    I2C_Helper_Write_Len(AHT20_ADDR, 0xBE, 2, Command);
    return 0;
}
u8 ATH20_Read_Status() // 读取AHT10的状态寄存器
{
    uint8_t Byte_first;
    I2C_Helper_ReadByte(AHT20_ADDR, 0x00, &Byte_first);
    return Byte_first;
}
u8 AHT20_StartAquire()
{
    u8 Command[2] = {0x33, 0x00};
    return I2C_Helper_Write_Len(AHT20_ADDR, 0xAC, 2, Command);
}
...
u8 AHT20_GetValue(AHT20_value *value)
{
    ...
    if (0 == AHT20_StartAquire()) {
        Delay_ms(100);
        //  读取结果
        if (0 == I2C_Helper_Read_Len(AHT20_ADDR, 0x00, 7, Buff)) {
            tmp |= Buff[1];
            tmp <<= 8;
            tmp |= Buff[2];
            tmp <<= 8;
