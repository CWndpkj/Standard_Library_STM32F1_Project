...
int8_t W25Qxx_Helper_Read(u32 reg_addr, u8 *pBuff, u16 length)
{
    ...
    if (W25Qxx_Type == W25Q256)
        for (u8 i = 0; i < 4; i++) {
            ReadCommand[i + 1] = reg_addr & ((u32)0xFF000000 >> i * 8);
        }
    else
        for (u8 i = 0; i < 3; i++) {
            ReadCommand[i + 1] = reg_addr & ((u32)0xFF0000 >> i * 8);
        }

    SPI_Helper_Start(W25Qxx_Cs_Pin);
    if (W25Qxx_Type == W25Q256) {
        if (SPI_Helper_WriteLen(ReadCommand, 5))
            ErrCode = 1;
    } else {
        if (SPI_Helper_WriteLen(ReadCommand, 4))
            ErrCode = 1;
    }
...
int8_t W25Qxx_Helper_Write(u32 reg_addr, u8 *pBuff, u16 length)
{
    u8 ErrCode = 0;

    // 计算偏移量
    u32 sector, page, block; // sector 0~15,block 0~127(W25Q64) 0~255(W25Q128) 0~511(W25Q256) page 0~256
    block  = reg_addr >> 16;
    sector = (reg_addr >> 12) & 0x0F;
    page   = reg_addr & 0xFF;

    // 先擦除
    for (u8 i = 0; i < length / 256 + 1; i++) {
        ErrCode = W25Qxx_Erase(reg_addr + 4096 * i);
    }
