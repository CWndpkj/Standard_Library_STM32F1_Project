...
W25Qxx_Helper W25Q64_Data(GPIO_Pin_6);
/*******************************************************************
 ...
 * @retvalue   :None
******************************************************************************/
void Gui_StrCenter(u16 x, u16 y, u16 fc, u16 bc, const char *str, u8 size, u8 mode)
{
...
void Gui_DrawPic(u16 x, u16 y, u16 Size_x, u16 Size_y, const unsigned char *p)
{
    // unsigned char picH, picL;
    LCD_SetWindows(x, y, x + Size_x - 1, y + Size_y - 1); // 窗口设置
    ...
    // }
    LCD_RS_SET;
    SPI_LCD.WriteLen((u8 *)p, Size_x * Size_y * 2);

    LCD_SetWindows(0, 0, lcddev.width - 1, lcddev.height - 1); // 恢复显示窗口为全屏
}

void Gui_LoadPicFromW25Qxx()
{
    W25Q64_Data.Init();
    u8 Buff[128];
    LCD_SetWindows(0, 0, 200 - 1, 200 - 1); // 窗口设置
    // for (i = 0; i < Size_x * Size_y; i++) {
    //     picL = *(p + i * 2); // 数据低位在前
    //     picH = *(p + i * 2 + 1);
    //     Lcd_WriteData_16Bit(picH << 8 | picL);
    ...
    for (u16 i = 0; i < 625; i++) {
        W25Q64_Data.Read(24576 * 3 + i * 128, Buff, 128);
        SPI_LCD.WriteLen(Buff, 128);
    }
